...
// MUTEX, Conditional wait, Thread Initializations
...
// Variables used in Statistics calculations
...
int TOK=0;	// Current no. of tokens in a bucket
int token_no=0; // Total tokens 
int token_dropped=0; //no. of tokens dropped
int packet_dropped=0; // no. of packets dropped bcoz of P>B
// After pressing Ctrl+C
int removed_packets=0; // no. of packets removed for Q1, Q2
int server_count=0; // no. of completed packets/ serviced packets

// Flags to wake up cond_wait at server
int stop=0;
int flag=0;
// Flag when all packets are dropped
int drop_flag=0;

// Flags to keep count of leaving packests at each facility
int last_q1=0;
int last_q2=0;
...
// Func descriptors
void *trace_arrival(); // Packet arrival if file is used
void *arrival(); // Packet arrival thread if file not used
void *tokendeposit(); // Token deposit thread
void *s1(); //server 1
void *s2(); //server 2
...
// Drop is used to show all the removed packets when terminated abruptly
void drop(); 


//Thread when file is used
void *trace_arrival()
{
	My402ListElem *elem=NULL; //Q1
	My402ListElem *elem2=NULL; //Q2
    packet *current=NULL;
	...
	while((fgets(send_buf, sizeof(send_buf), fp)) != NULL) 
	{
      ...
      if(atof(value[0])==0)
        		{
					...
					{
        				fprintf(stderr, "entered packet arrival is not a real value in line %d\n",i+2);
            			exit(-1);
        			}
        		}
      ...
      service_time=atof(value[2])/1000.00;
       ...
       if(atof(value[2])==0)
		...
		inter_arrival = (t2-t)*1000-packet_time;//  current entry-previous entry
		Total_piat = Total_piat + inter_arrival;// Total interarrival for all packets
		...
		pthread_mutex_lock(&m);
			printf("%012.03fms: p%d arrives, needs %d tokens, inter-arrival time = %0.03fms\n", (t2-t)*1000, newpacket->count, newpacket->tokens, inter_arrival);
		...
		pthread_mutex_lock(&m); //lock
			...
			My402ListAppend(&Q1,(void*)newpacket); // add to Q1
			q1++;
			printf("%012.03fms: p%d enters Q1\n", (t3-t)*1000, newpacket->count);
			elem=My402ListFirst(&Q1);
			current=(packet *)(elem->obj);
			if(current->tokens<=TOK ) // remove the required tokens from token bucket
			{
				
				for(k=0; k<current->tokens; k++)
				{
					elem2=My402ListFirst(&Bucket);
					TOK--;
					My402ListUnlink(&Bucket,elem2);
				}
				My402ListUnlink(&Q1,elem); // removes from q1
				last_q1 ++;
				gettimeofday(&timer, NULL);
				t4 = timer.tv_sec  + timer.tv_usec/1000000.00;
				printf("%012.03fms: p%d leaves Q1, time in Q1 = %0.03fms, token bucket now has %d tokens\n", (t4-t)*1000, current->count, (t4-t)*1000-current->start_timeinq1, TOK);
				...
				My402ListAppend(&Q2, (void*)current);// enters q2
	...
	{// Packets dropped
		pthread_mutex_lock(&m); //lock
			printf("%012.03fms: p%d arrives, needs %d tokens, inter-arrival time = %0.03fms, dropped\n", (t2-t)*1000, newpacket->count, newpacket->tokens, inter_arrival);
		pthread_mutex_unlock(&m);
		packet_dropped++;
		actual_packet--;
		if(packet_dropped==n)
		{
			...
			My402ListUnlinkAll(&Bucket);
			pthread_cancel(token_arrival_thread);
		}
		if(My402ListLength(&Q1)==0&&newpacket->count==n)
		{
			stop=1;
			pthread_cond_broadcast(&cv);
			My402ListUnlinkAll(&Bucket);
...
void *arrival()
{
	...
	for(i=0; i<n; i++)
	{
		...
		inter_arrival = (t2-t)*1000-packet_time; // present entry-previous entry
		...
		pthread_mutex_lock(&m);
			...
			My402ListAppend(&Q1,(void*)newpacket); // enter q1
			q1++;
			printf("%012.03fms: p%d enters Q1\n", (t3-t)*1000, newpacket->count);
			elem=My402ListFirst(&Q1);
			current=(packet *)(elem->obj);
			if(current->tokens<=TOK ) 
			{
				...
				for(k=0; k<current->tokens; k++) // to receive the required tokens from bucket
				{
					elem2=My402ListFirst(&Bucket);
					TOK--;
					My402ListUnlink(&Bucket,elem2);
				}
				My402ListUnlink(&Q1,elem); // remove from q1
				last_q1 ++;
				gettimeofday(&timer, NULL);
				t4 = timer.tv_sec  + timer.tv_usec/1000000.00;
				printf("%012.03fms: p%d leaves Q1, time in Q1 = %0.03fms, token bucket now has %d tokens\n", (t4-t)*1000, current->count, (t4-t)*1000-current->start_timeinq1, TOK);
				...
				t5 = timer.tv_sec  + timer.tv_usec/1000000.00;
				current->start_timeinq2=(t5-t)*1000;
				My402ListAppend(&Q2, (void*)current);
				printf("%012.03fms: p%d enters Q2\n", (t5-t)*1000, current->count); // enters q2
...
void *tokendeposit()
{
	...
	if(TOK<B) // Check the capacity
	{
		...
		pthread_mutex_lock(&m); //lock
			...
			My402ListAppend(&Bucket,(void *)token_no);// add in the bucket
			...
			if(current->tokens<=TOK )// Q1 takes required tokens
			{
				...
				printf("%012.03fms: p%d leaves Q1, time in Q1 = %0.03fms, token bucket now has %d tokens\n", (t7-t)*1000, current->count, (t7-t)*1000-current->start_timeinq1, TOK);
				time_q1=(t7-t)*1000-current->start_timeinq1; // Total time in q1
				current->timeinq1=time_q1;
				gettimeofday(&timer, NULL);
				t8 = timer.tv_sec  + timer.tv_usec/1000000.00;
				current->start_timeinq2=(t8-t)*1000;
				My402ListAppend(&Q2, (void*)current); //enters q2
				printf("%012.03fms: p%d enters Q2\n", (t8-t)*1000, current->count);
				pthread_cond_broadcast(&cv);
			}
...
pthread_mutex_lock(&m);
	...
	while(My402ListLength(&Q2)==0) // Conditional wait
	{
		if(stop==1)
		{
			flag=1;
	...
	if(My402ListLength(&Q2)!=0)
	{
		...
		My402ListUnlink(&Q2,elem); // leaves q2
		last_q2++;
		gettimeofday(&timer, NULL);
		t10 = timer.tv_sec  + timer.tv_usec/1000000.00;
		printf("%012.03fms: p%d leaves Q2, time in Q2 = %.03fms\n", (t10-t)*1000, current->count, (t10-t)*1000-current->start_timeinq2);
	...
	pthread_mutex_unlock(&m);
		...
		printf("%012.03fms: p%d begins service at s1, requesting %0.03fms of service\n", (t11-t)*1000, current->count, current->service);
		usleep(s_time*1000); // servicing
		gettimeofday(&timer, NULL);
		t12 = timer.tv_sec  + timer.tv_usec/1000000.00;
		printf("%012.03fms: p%d departs from s1, service time = %.03fms, time in system = %0.03fms\n", (t12-t)*1000, current->count, (t12-t)*1000-(t11-t)*1000, (t12-t)*1000-current->start_timeinq1);
		...
		Total_sqtimeinsystem=Total_sqtimeinsystem+(timeinsys*timeinsys); // Total squared time in system
...
while(1)
{
	...
	if(My402ListLength(&Q2)!=0)
	{
		...
		current=(packet *)(elem->obj);
		s_time=current->service;
		My402ListUnlink(&Q2,elem); // remove from q2
		last_q2++;
		gettimeofday(&timer, NULL);
		t13 = timer.tv_sec  + timer.tv_usec/1000000.00;
		printf("%012.03fms: p%d leaves Q2, time in Q2 = %.03fms\n", (t13-t)*1000, current->count, (t13-t)*1000-current->start_timeinq2);
	...
	pthread_mutex_unlock(&m);
		...
		printf("%012.03fms: p%d begins service at s2, requesting %0.03fms of service\n", (t14-t)*1000, current->count, current->service);
		usleep(s_time*1000);
		gettimeofday(&timer, NULL);
		t15 = timer.tv_sec  + timer.tv_usec/1000000.00;
		printf("%012.03fms: p%d departs from s2, service time = %.03fms, time in system = %0.03fms\n", (t15-t)*1000, current->count, (t15-t)*1000-(t14-t)*1000, (t15-t)*1000-current->start_timeinq1);
		...
		Total_timeinsystem=Total_timeinsystem+timeinsys; // total time in system
		Total_sqtimeinsystem=Total_sqtimeinsystem+(timeinsys*timeinsys); // Total sq time in sys
		server_count++; // completed packets
...
avg_q1=(Total_time_q1/((end-t)*1000)); // average number of packets in Q1
avg_q2=(Total_time_q2/((end-t)*1000)); // average number of packets in Q2
avg_s1=(Total_time_s1/((end-t)*1000)); // average number of packets in S1
avg_s2=(Total_time_s2/((end-t)*1000)); // average number of packets in S2

avg_tis=(Total_timeinsystem/server_count); // average time a packet spent in system
avg_sqtis=(Total_sqtimeinsystem/server_count);// average squared time a packet spent in system
var_tis=avg_sqtis-(avg_tis*avg_tis); // var of average time a packet spent in system
if(var_tis==0.00||var_tis<0.00)
flag_var=1;
std_tis=sqrt(var_tis); // standard deviation of average time a packet spent in system
...
{
printf("     average number of packets in Q1 = N/A, Total Emulation time is zero\n");
printf("     average number of packets in Q2 = N/A, Total Emulation time is zero\n");
printf("     average number of packets at S1 = N/A, Total Emulation time is zero\n");
printf("     average number of packets at S2 = N/A, Total Emulation time is zero\n\n");
}
if(server_count)
{
...
{
printf("     average time a packet spent in system = N/A, no packets were processed at the facilities s1 and s2\n");
printf("     standard deviation for time spent in system = N/A, no packets were processed at the facilities s1 and s2\n\n");
}
if(token_no)
printf("     token drop probability = %0.6g\n", token_drop_prob);
else
printf("     token drop probability = N/A, No tokens generated at this facility\n");
if(n-removed_packets)
printf("     packet drop probability = %0.6g\n", packet_drop_prob);
else
printf("     packet drop probability = N/A, No packets arrived at this facility\n");
...
void drop()
{
  ...
  listq1=My402ListLength(&Q1);
  listq2=My402ListLength(&Q2);
...
void *monitor() 
{
  ...
  pthread_mutex_lock(&m);
    ...
    pthread_cancel(token_arrival_thread);
    removed_packets=My402ListLength(&Q1)+My402ListLength(&Q2);
    printf("\n");
    drop_flag=1;
    My402ListUnlinkAll(&Bucket);
  pthread_mutex_unlock(&m);
...
int main(int argc, char *argv[])
{
    ...
    // signal catching initializations
    sigemptyset(&set);
	...
	for(i=0; i<argc; i++)
	{
		...
		{ 
			...
			if(strcmp(argv[i],"-t")==0)
			{
				if(argv[i+1]==NULL)
				{
					fprintf(stderr, "No Argument entered for filename\n");
            		exit(-1);
			...
			else if(strcmp(argv[i],"-n")==0)
			{
				...
				if(n==0)
        		{
					...
					{
        				fprintf(stderr, "Argument entered for number of packets is not an integer\n");
            			exit(-1);
        			}
        		}	
			...
			else if(strcmp(argv[i],"-lambda")==0)
			{
				...
				if(lambda==0)
        		{
					...
					{
        				fprintf(stderr, "Argument entered for lambda is not a real value\n");
            			exit(-1);
        			}
        		}	
			...
			else if(strcmp(argv[i],"-B")==0)
			{
				...
				if(B==0)
        		{
					...
					{
        				fprintf(stderr, "Argument entered for max number of tokens is not an integer\n");
            			exit(-1);
        			}
        		}	
			...
			else if(strcmp(argv[i],"-P")==0)
			{
				...
				if(P==0)
        		{
					...
					{
        				fprintf(stderr, "Argument entered for number of tokens per packet is not an integer\n");
            			exit(-1);
        			}
        		}	
			...
			{
				fprintf(stderr, "The command arguments provided by the user is invalid\nThe arguments can either be:\n -n, -lambda, -mu, -P, -r, -B or -t and their respective real values\nQuit Execution\n");
				exit(-1);
			}
			}
			else
	...
	if(trace==1)
	{
		...
		pthread_create(&packet_arrival_thread, 0, trace_arrival, NULL);
	...
	pthread_create(&packet_arrival_thread, 0, arrival, NULL);

	}

	pthread_create(&token_arrival_thread, 0, tokendeposit, NULL);
